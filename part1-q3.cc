...
//Question 1 part 3: base program was taken from tcp-bbr-example.cc and modified for the project requirements.
...
// links are 3 Mbps.
//
// This program runs by default for 200 seconds and creates a new directory
// called 'part1-q3-results' in the ns-3 root directory. 
...
RxDrop (Ptr<const Packet> p)
{
  ...
  //*dropstream->GetStream () << Simulator::Now ().GetSeconds () <<std::endl;
...
int main (int argc, char *argv [])
{
  ...
  Config::SetDefault ("ns3::DropTailQueue<Packet>::MaxSize", QueueSizeValue (QueueSize ("10000p")));
  ...
  edgeLink.SetDeviceAttribute  ("DataRate", StringValue ("5Mbps"));
  ...
  r1r2.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em1));
  ...
  //Simulator::Schedule (Seconds (0.2), &TraceDrop, 1, 0);  //node 1 is receiver
  ...
  dir = "part1-q3-results/" + currentTime + "/";
 ...
 //for plotting through gnuplot:

  //system (("cp -R PlotScripts/gnuScriptCwnd " + dir).c_str ());
  system (("cp -R PlotScripts/gnuScriptRtts " + dir).c_str ());
  system (("cp -R plotscripts/gnuScriptThruput " + dir).c_str ());
  

  

  receiverEdge.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop));
