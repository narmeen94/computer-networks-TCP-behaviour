/* -*- Mode:C++; c-file-style:"gnu"; indent-tabs-mode:nil; -*- */
 ...
 //question 2 part 1: the base program is tcp-validation.cc and has been modified for project requirements.
...
NS_LOG_COMPONENT_DEFINE ("part2-q1");
...
main (int argc, char *argv[])
{
  ...
  std::string firstTcpThroughputTraceFile = "tcp-throughput.dat";
  
  std::string secondTcpThroughputTraceFile = "udp-throughput.dat";
  ...
  Config::SetDefault ("ns3::RedQueueDisc::MaxSize",QueueSizeValue (QueueSize (QueueSizeUnit::PACKETS, 10)));
  ...
  //bottleneckLink.SetQueue("ns3::RedQueue","MaxSize", QueueSizeValue (QueueSize ("100p")));
...
//assigning addresses:
  ...
  // Configure  TCP client/server pair
  ...
  // Configure UDP client/server pair
  //using onoff helper as bulksource helper did not allow changing of data rate (traffic generation rate)

   OnOffHelper udp1 ("ns3::UdpSocketFactory",Address());
   uint16_t secondPort = 5000;
   InetSocketAddress secondDestAddress (secondClientIfaces.GetAddress (1), secondPort);
   udp1.SetAttribute ("Remote", AddressValue (secondDestAddress));
   udp1.SetConstantRate (DataRate ("1Mbps"));
   

  ApplicationContainer secondApp;
  secondApp = udp1.Install (Server2);
  secondApp.Start (Seconds(0.0));   //second flow will start after 1 second
  secondApp.Stop (stopTime);

  Address secondSinkAddress (InetSocketAddress (Ipv4Address::GetAny (), secondPort));
  PacketSinkHelper secondSinkHelper ("ns3::UdpSocketFactory", secondSinkAddress);
 ...
 // Setup scheduled traces; TCP traces must be hooked after socket creation
  ...
  Simulator::Schedule (Seconds (1.0) + MilliSeconds (100), &ScheduleSecondPacketSinkConnection);
