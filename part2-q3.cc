/* -*- Mode:C++; c-file-style:"gnu"; indent-tabs-mode:nil; -*- */
 ...
 //question 2 part 3: the base program is tcp-validation.cc and has been modified for project requirements.
...
NS_LOG_COMPONENT_DEFINE ("part2-q3");
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::TcpSocketBase::MaxWindowSize", UintegerValue (30*1448));
  ...
  //both the flows have same type of tcp flow. 

  TypeId firstTcpTypeId= TcpNewReno::GetTypeId ();
  
  TypeId secondTcpTypeId=TcpNewReno::GetTypeId ();
  ...
  //for setting the packet size of flow 1 to 1000 bytes . SendSize here = segment size. 
  //in tcp packet size and segment size are same so, we can set attribute using the helper application
  tcp1.SetAttribute ("SendSize", UintegerValue (1000));
  ...
  tcp1.SetAttribute ("MaxBytes", UintegerValue (0));
  //setting the packet size of flow 2 to 500 bytes
  tcp1.SetAttribute ("SendSize", UintegerValue (500));
